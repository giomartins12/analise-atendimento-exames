...
O instalador `AnaliseAtendimentoExames-Setup-0.1.2-win64.exe` contém o runtime Python e todas as dependências. O computador de destino não precisa ter Python instalado.
...
Se a interface não abrir, use **Diagnóstico - Análise de Atendimento e Exames** no menu Iniciar. A versão 0.1.2 elimina o cadastro interativo de e-mail do Streamlit, abre o navegador pelo launcher e grava `application.log`, `console.log` e `native-crash.log` em `%LOCALAPPDATA%\AnaliseAtendimentoExames`.
